...
records %>%
  ...
  labs(title="Time Distribution by Type of Track")
#  geom_histogram(stat='identity', bins=50, alpha=.5) +
#  scale_x_discrete(label=function(x) stringr::str_trunc(x,10))
```
Record times for Three Lap tracks are typically much longer than those for Single Lap tracks, although their bins do overlap, and the smallest bin on the histogram only contains data from Three Laps. This makes sense because completing more laps usually implies a longer game. Also, Three Lap tracks have a much wider range of observed times, whereas the times for Single Lap tracks are mostly under 100 seconds.
